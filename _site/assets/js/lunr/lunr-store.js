var store = [{
        ...
        "excerpt":"Pipenv 를 이용하여 가상환경 Setting pipenv 란? → python 에서 사용하는 가상환경을 구성하는 방식 중 하나. pipenv 를 사용하는 이유 → 프로젝트별로 다른 패키지를 사용하며, 같은 패키지를 사용하더라도 버전이 다른 경우가 있기 때문에 pipenv 로 가상환경을 만든다. 설치방법 → pipenv 를 설치하기 전에 python 을 설치 해야 함. 아래 부터는...","categories": ["Django","Python"],
        "tags": [],
        "url": "/django/python/Django-Setting/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Django, Nginx, Gunicorn Setting (CentOS)",
        "excerpt":"Python3 설치 yum 으로 python3 설치 yum install python3 -y yum 으로 python3 을 설치하면 pip3 도 같이 설치됨 python3 를 입력. 아래와 같이 나오면 설치 완료 설치하고자 하는 프로젝트 경로에 가상환경 설치 /usr/local/thriller/thriller pip3 install virtualenv virtualenv venv 가상환경에 접속 source venv/bin/activate 가상환경에서 빠져나옴 deactivate MySQL DataBase, 사용자 생성...","categories": ["Django","Nginx","Gunicorn","Linux"],
        "tags": [],
        "url": "/django/nginx/gunicorn/linux/Django-Nginx-Gunicorn/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Django Forms",
        "excerpt":"Django Form 장고에는 Model 클래스를 이용하여 Form 을 자동으로 생성하는 기능이 있다. 장고의 폼에는 is_valid() 라는 함수가 있다 is_valid() 란? ⇒ 입력받은 폼에 대한 유효성을 검사 views.py from django.views import View from django.shortcuts import render from . import forms class LoginView(View): def get(self, request): form = forms.LoginForm(initial={\"email\": \"\"}) return render(request,...","categories": ["Django","Python"],
        "tags": [],
        "url": "/django/python/Django-Forms/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Django User Settings",
        "excerpt":"AUTH_USER_MODEL 설정 장고에는 기본적으로 내장되어있는 AbstractUser 이 있다. 내장된 AbstractUser 를 사용할 수도 있지만 보통은 새롭게 정의해서 사용하기 때문에 아래 설정을 settings.py 파일에서 해준다. → 아래 설정을 생략하면 장고의 기본적으로 세팅되어있는 user 모델과 혼동이 올 수 있음 AUTH_USER_MODEL = 'users.User' LOGIN_URL, LOGIN_REDIRECT_URL 설정 Django 의 Login, Logout 메커니즘은 next 라는...","categories": ["Django","Python"],
        "tags": [],
        "url": "/django/python/Django-User-Settings/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "jekyll 로 fork 해온 git blog 테마, 잔디밭 안심어질 때",
        "excerpt":"현재상태 git blog 를 생성하면서 jekyll 테마를 fork 해서 내 repository 에 생성을 했었는데 잔디밭이 안심어지는 현상을 발견! (이왕이면 심어지면 좋으니까..) 아마 내 repository 가 아닌 다른 사람의 repository 에 push 되고 있는게 아닐까 생각.. 해결방안 github 에 새로운 repository 를 생성 (new_blog) 기존에 있는 repository 를 bare clone 한다...","categories": ["Git","Gitblog","Github"],
        "tags": [],
        "url": "/git/gitblog/github/fork-gitblog/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "python3, pip3 Symbolic Link 설정",
        "excerpt":"현재상태 Linux 에는 기본적으로 Python2 버전이 설치된다. CentOS 에서 Python3 버전을 설치했는데 python 을 입력할 경우 2.x 버전으로 인식한다. 매번 python3 으로 명령어를 입력하는게 불편하여 구글링 해보니 Symbolic Link 를 통해 해결할 수 있다고 한다. 해결방안 python3 설정 # 현재 심볼릭 링크 확인 ls -l /bin/python* # lrwxrwxrwx. 1 root...","categories": ["Python","Linux"],
        "tags": [],
        "url": "/python/linux/python3-pip3-symbolic-link/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Optional 이란",
        ...
        "title": "테스트 코드",
        "excerpt":"테스트 코드란 프로그램 작성 시 문제가 없는지 확인하기 위해 사용 테스트 코드를 작성하는 이유 빠른 피드백 자동검증이 가능 개발자가 만든 기능을 안전하게 보호해 준다. 테스트 코드 작성 방법 → 메서드를 작성하고 그 위에 @Test 를 작성하면 된다. → 테스트코드는 직관적으로 보기 위해 메소드를 한글로 작성하기도 한다 → 테스트코드는 빌드 시...","categories": ["Java"],
        "tags": [],
        "url": "/java/Java-TESTCODE/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "컴포넌트 스캔과 자동 의존관계",
        "excerpt":"의존관계주입(DI) 이란? 객체 간의 의존 관계를 만드는 것 Spring 의 IOC 컨테이너의 구체적인 구현 방식 의존관계 설정 컨트롤러(Controller)가 서비스(Service)와 리포지토리(Repository)를 사용할 수 있게 의존관계를 준비 → 컨트롤러에 @Controller 만 붙인다고 service 와 repository 를 사용할 수 있는게 아니다 controller/MemberController.java package hellospring.hello.controller; import hellospring.hello.service.MemberService; import org.springframework.beans.factory.annotation.Autowired; import org.springframework.stereotype.Controller; @Controller public class...","categories": ["Java"],
        "tags": [],
        "url": "/java/Java-ComponentScan-DI/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Transform",
        ...
        "excerpt":"엔티티 매니저 (EntityManager) 엔티티를 저장, 수정, 삭제, 조회 등 엔티티와 관련된 작업을 수행 @PersistenceContext 를 통해 EntityManager 를 주입 받아 사용 @Repository public class Repository { @PersistenceContext EntityManager em; em.find(); // 엔티티 조회 em.persist(); // 엔티티 저장 em.remove(); // 엔티티 삭제 em.flush(); // 영속성 컨텍스트 내용을 데이터베이스에 반영 em.detach(); //...","categories": ["Java"],
        "tags": [],
        "url": "/java/Java-EntityManager/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Transactional 이란",
        "excerpt":"트랜잭션이란? 데이터베이스의 상태를 변경하는 작업 한번에 수행되어야 하는 연산들을 의미 begin, commit 을 자동으로 수행 예외 발생 시 rollback 처리를 자동으로 수행 트랜잭션의 4가지 성질 원자성 → 한 트랜잭션 내에서 실행한 작업들은 하나의 단위로 처리함. 즉, 모두 성공 또는 모두 실패 일관성 → 일관성 있는 데이터베이스 상태를 유지 격리성 →...","categories": ["Java"],
        "tags": [],
        "url": "/java/Java-Transactional/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        ...
        "title": "변경 감지와 병합 - 첫번째",
        "excerpt":"준영속 이란? → 영속성 컨텍스트가 관리하는 영속 상태의 엔티티가 영속성 컨텍스트에서 분리된(detached) 것을 준영속 상태라 한다 영속 상태 엔티티를 준영속 상태로 만드는 방법 em.detach(entity): 특정 엔티티만 준영속 상태로 전환 em.clear(): 영속성 컨텍스트를 완전히 초기화 em.close(): 영속성 컨텍스트를 종료 준영속 엔티티란? 영속성 컨텍스트가 더는 관리하지 않는 엔티티를 말한다 준영속이라는 단어는 객체를...","categories": ["Java","JPA"],
        "tags": [],
        "url": "/java/jpa/Java-DirtyChecking1/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "엔티티 매핑",
        "excerpt":"JPA 를 사용하는 데 가장 중요한 일은 엔티티와 테이블을 정확히 매핑하는 것. 따라서, 매핑 어노테이션을 숙지하고 사용해야 함 대표 어노테이션 객체와 테이블 매핑: @Entity, @Table 기본 키 매핑: @Id 필드와 컬럼 매핑: @Column 연관관계 매핑: @ManyToOne, @JoinColumn @Entity JPA 를 사용해서 테이블과 매핑할 클래스는 @Entity 어노테이션을 필수로 붙여야함 @Entity 가...","categories": ["Java","JPA"],
        "tags": [],
        "url": "/java/jpa/Java-Entity-Mapping/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "변경감지와 병합 - 두번째",
        "excerpt":"SQL 수정(Update) 쿼리의 문제점 SQL 을 사용하면 수정 쿼리를 직접 작성 프로젝트의 규모가 커지고 요구사항이 늘어나면 수정쿼리도 점점 추가된다. → 이 방식의 문제점은 수정 쿼리가 많아지는 것과 비지니스 로직을 분석하기 위해 SQL 을 계속 확인해야함. 결국, 직접적이든 간접적이든 비지니스 로직이 SQL 에 의존하게 됨 // 초기 update member set name...","categories": ["Java","JPA"],
        "tags": [],
        "url": "/java/jpa/Java-DirtyChecking2/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "JPA Repository",
        "excerpt":"ItemRepository.java package com.shop.shop.repository; import com.shop.shop.entity.Item; import org.springframework.data.jpa.repository.JpaRepository; public interface ItemRepository extends JpaRepository&lt;Item, Long&gt; { } JpaRepository JpaReporitory 는 2개의 제네릭 타입을 사용하는데 첫번째에는 엔티티 타입 클래스, 두번재는 기본키 타입을 넣어줌 JpaReporitory 는 기본적인 CRUD 및 페이징 처리를 위한 메소드가 정의돼 있음 JpaReporitory 에서 지원하는 메소드 예시 application-test.properties # Datasource 설정...","categories": ["Java","JPA"],
        "tags": [],
        "url": "/java/jpa/Java-JPARepository/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Nginx 를 이용하여 SpringBoot 무중단 배포하기",
        "excerpt":"참고) 스프링 부트와 AWS로 혼자 구현하는 웹서비스 [SpringBoot] 웹서비스 출시하기 - 5. Nginx를 활용한 무중단 배포 구축하기 1. 구조 Nginx 1대, 스프링부트 jar 2대 Nginx에는 80(http), 443(https) 포트를 할당합니다. 스프링부트 jar1에는 8081포트로 , 스프링부트 jar2에는 8082포트로 실행합니다.(포트는 원하시는 포트를 사용하시면 됩니다.) 구조는 아래의 그림같이 형성 위 그림의 동작 과정 사용자는...","categories": ["Java","Nginx","SpringBoot"],
        "tags": [],
        "url": "/java/nginx/springboot/Java-Nginx-SpringBoot/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
        "title": "Spring Profile 이란?",
        "excerpt":"개발환경에 따라 설정파일을 다르게 로딩해야할 필요가 있다 이처럼 Profile 은 어떤 특정 환경의 설정 값을 다르게 하고 싶을 때 사용 예를 들어, 테스트 환경과 배포 환경을 다르게 두고 Profile 설정 기본적인 profile 정보 application.properties application.yml 여기서 특정 규칙에 만족하게 설정 파일을 만들면 SpringBoot 가 읽어올 수 있다. application-{프로필네임키워드}.properties ex. application-dev.properties...","categories": ["Java","SpringBoot"],
        "tags": [],
        "url": "/java/springboot/Java-SpringProfile/",
        "teaser": "/assets/images/IMG_2153.JPG"
      },{
