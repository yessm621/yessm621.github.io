...
@ModelAttribute는 생략 가능함. 그런데 @RequestParam도 생략 가능하므로 혼란이 발생
...
**InputStream(Reader)**: HTTP 요청 메시지 바디의 내용을 조회
...
- HttpServletRequest를 사용해 직접 HTTP 메시지 바디에서 데이터를 읽어와서 문자로 변환함
